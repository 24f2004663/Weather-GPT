...
class ExotelVoiceAdapter(BaseNotificationAdapter):
    """
    Exotel Emergency Voice/IVR Call Notification Adapter.
    Initiates emergency outbound notification calls with audio prompt/TTS scripts.
    """
    def __init__(
        self,
        ...
        self.base_url = f"https://{domain}/v1/Accounts/{self.account_sid}/Calls/connect.json" if self.account_sid else None
    ...
    async def send_notification(self, payload: NotificationPayload) -> DeliveryStatus:
        ...
        if self.dry_run:
            logger.info(f"[SIMULATION: EXOTEL VOICE] To: {clean_recipient} | Spoken Script: {payload.message[:60]}...")
            return DeliveryStatus(
                notification_id=notification_id,
                channel=NotificationChannel.VOICE_IVR,
                recipient=clean_recipient,
                status=NotificationStatus.SIMULATED,
                provider_reference=f"sim_call_{uuid.uuid4().hex[:12]}",
        ...
        if not self.account_sid or not self.api_key or not self.api_token or not self.caller_id:
            logger.warning("Exotel Voice credentials or Caller ID not configured.")
            return DeliveryStatus(
                notification_id=notification_id,
                channel=NotificationChannel.VOICE_IVR,
                recipient=clean_recipient,
                status=NotificationStatus.FAILED,
                error_message="Exotel Voice credentials or Caller ID not configured.",
                timestamp=datetime.utcnow()
            )
        ...
        try:
            ...
            logger.error(f"Exotel Voice call timed out to {clean_recipient}")
            return DeliveryStatus(
                notification_id=notification_id,
                channel=NotificationChannel.VOICE_IVR,
                recipient=clean_recipient,
                status=NotificationStatus.RETRYING,
                error_message="Exotel Voice call timed out.",
                timestamp=datetime.utcnow()
            )
        except Exception as e:
            logger.error(f"Network error initiating Exotel call: {str(e)}")
            return DeliveryStatus(
                notification_id=notification_id,
                channel=NotificationChannel.VOICE_IVR,
                recipient=clean_recipient,
        ...
        if response.status_code in [200, 201]:
            ...
            call_data = resp_data.get("Call", {})
            sid = call_data.get("Sid", "exotel_call_initiated")
            return DeliveryStatus(
                notification_id=notification_id,
                channel=NotificationChannel.VOICE_IVR,
                recipient=clean_recipient,
            ...
            return DeliveryStatus(
                ...
                recipient=clean_recipient,
                status=NotificationStatus.FAILED,
                error_message=f"Exotel Voice API returned HTTP {response.status_code}: {response.text[:100]}",
                timestamp=datetime.utcnow()
            )

exotel_voice_adapter = ExotelVoiceAdapter()
