...
class GeminiAIService(BaseAIService):
    """
    Production-grade Google Gemini AI Orchestration Layer with Per-Request Quota Routing.
    
    Guarantees:
    - 1 Actual Gemini HTTP POST = 1 Quota Reservation.
    - Every HTTP call (initial prompt or tool-return) selects the highest-priority eligible model.
    - If a model reaches RPM limit between tool iterations, the loop seamlessly switches models.
    - On HTTP 429, the model is suppressed for 60s and the request retries with the next model.
    - Complete conversational context, session history, and tool deduplication are preserved.
    """
    def __init__(
        self,
    ...
    async def generate_weather_response(self, request: ChatRequest) -> ChatResponse:
        """
        Executes conversational AI with per-HTTP-request quota routing and function calling.
        """
        ...
        excluded_models_for_request: Set[str] = set()

        # 3. Controlled Function Calling Tool Loop with Per-HTTP-Request Quota Reservation
        for iteration in range(self.max_tool_iterations):
            # Estimate tokens for this specific upcoming request
            ...
            for model_attempt in range(max_model_fallbacks):
                ...
                logger.info(f"[Gemini API] event=request model={clean_model} tool_iteration={iteration}")
                ...
                if response.status_code == 429:
                    ...
                    # Retry this iteration with next model in cascade
                    continue
            ...
            if response.status_code != 200:
                clean_model = selected_model_config.name if selected_model_config else "gemini"
            ...
            try:
                ...
                raise UpstreamProviderError(
                    provider="Gemini API", status_code=200, message="Malformed JSON response from Gemini API"
                )
